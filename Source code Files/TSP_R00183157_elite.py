...
class BasicTSP:
    def __init__(self, _fName, _popSize, _mutationRate, _maxIterations, _config, _elPercentage):
        """
...
#            ****Single Child Implementation**********
            ...
            self.inversionMutation(childobj)
            self.updateBest(childobj)
            self.newGeneration.append(childobj)
        self.newGeneration.sort(key=lambda x: x.fitness)        
        self.replace_values=int( float(self.ePercent) * self.popSize)
        #Replacing NewGeneration values into Population using Elite configuration
        for i in range(self.replace_values):
            self.population[i]=self.newGeneration[i]
...
for i in range(5):
    ...
    ga = BasicTSP(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4],sys.argv[5],sys.argv[6])
